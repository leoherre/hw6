...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
  ...
  //Looop thru table and delete anyything stored
...
{
  //resize if necessary
  if(numInserted/CAPACITIES[mIndex_] >= resizeAlpha_){
    resize();
  }

  HASH_INDEX_T checker = probe(p.first);
  if(checker == npos){
    throw std::logic_error("NO FREE SPACE");
  }
  //inserting and creating i space
  else if(table_[checker] == nullptr){
    table_[checker] = new HashItem(p);
    numInserted++;
    numPresent++;
  }
  //insert item 
  else {
    table_[checker]->item.second = p.second;
  }
}
...
{
  //find key and delete if it exists
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  ...
  //copy of table onto temp
  std::vector<HashItem*> temp = table_;
  table_.assign(CAPACITIES[mIndex_],nullptr);
  
  //reset values for keys 
  numPresent = 0;
  numInserted = 0;
  //loop thru table and find avaiable space, then insert
